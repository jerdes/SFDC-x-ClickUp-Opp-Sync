...
def run_sync(
    ...
    for opp in match.to_create:
        try:
            custom_fields = build_custom_fields_payload(opp, field_ids, dropdown_maps, text_canonicals)
    ...
    for opp, task in match.to_update:
        task_id = task["id"]
        try:
            changed_fields = get_changed_fields_payload(opp, task, field_ids, dropdown_maps, text_canonicals)
